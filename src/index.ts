...
app.post("/api/rooms/create", async (req, res) => {
  ...
  if (!req.body.userID) {
    res
      .status(400)
      .send("Couldn't create room because request is missing UserID.");
    return;
  }
  ...
  if (typeof req.body.userID != "string") {
    res
      .status(400)
      .send("Couldn't create room because because UserID is not a string.");
    return;
  }
  ...
  if (!req.body.accessKey) {
    res
      .status(400)
      .send("Couldn't create room because request is a access key.");
    return;
  }
  ...
  if (typeof req.body.accessKey != "string") {
    res
      .status(400)
      .send("Couldn't create room because because accessKey is not a string.");
    return;
  }
  ...
  if (!(await isAccessKeyValid(req.body.accessKey))) {
    res
      .status(400)
      .send("Couldn't create room because because accessKey is not valid.");
    return;
  }
...
function canUserJoinRoom(
  ...
  if (room) {
    ...
    if (
      room.users.length + 1 > room.settings.maxPlayer ||
      room.users.length + 1 > SERVER_MAX_PLAYERS_CHARADES
    ) {
...
function isAccessKeyAlreadyInUse(accessKey: string): boolean {
  let accessKeyInUse = active_rooms
    // TODO : why on earth is it done this way
    .map((room) =>
      room.users.map((user) => user.socket.data.accessKey == accessKey)
    )
    .flat()
    .includes(true);
...
io.on("connect", async (socket) => {
  ...
  socket.on("lobby:start_game", (_, callback) => {
    ...
    if (
      ...
      room.users.length < SERVER_MAX_PLAYERS_CHARADES
    ) {
      callback({
        success: false,
    ...
    let enoughCredits = !room.users
      .map(
        (roomUser) =>
          roomUser.initialCredits >
          room!.settings.roundCount * room!.lobbyState.selectedTopics.length
      )
      .includes(false);
  ...
  socket.on("game:submit_prompt", async (prompt, callback) => {
    ...
    // TODO : Prevent out of order insertion, if first call returns after the second one it will insert in wrong order
    imageGeneration(room.settings.aiModel, prompt).then(async (res) => {
      ...
      if (
        !(await attemptDecrementAccessKey(
          socket.data.accessKey,
          getCostForModel(room.settings.aiModel),
          MAX_DECREMENT_ACCESS_KEY_ATTEMPTS
        ))
      ) {
        room.state = "ERROR";
      ...
      // TODO : What if max player is more than 2, then this won't make sense
      // Should keep how many players started
  ...
  socket.on("game:submit_guess", (guess, callback) => {
    ...
    room.users.forEach((roomUser) => {
      if (
        roomUser.gameState.guessPlace < roomUser.gameState.imagesToGuess.length
      ) {
        everyoneFinished = false;
      }
    });
  ...
  socket.on("result:next_result", () => {
    ...
    if (room.resultState.resultPlace + 1 >= room.resultState.results.length) {
      return;
    }
    ...
    if (room.settings.nextResultPermission == "AUTHOR") {
      room.resultState.currentRevealer =
        room.resultState.results[room.resultState.resultPlace].prompter.userID;
  ...
  socket.on("disconnect", () => {
    ...
    room.users = room.users.filter((item) => item.userID != socket.data.userID);

    // Remove the room if no users
    if (room.users.length == 0) {
      active_rooms.filter((activeRoom) => activeRoom.roomID != room!.roomID);
      return;
    }
